...
def remove_eog_with_visualization(raw_signal, fs=250,Drawflag=0):
    ...
    """
    # ===== 参数配置 =====
    k = int(0.2 * fs)  # 长时差分延迟(160ms)
    wavelet = 'sym5'  # 小波基
    J = 6  # 小波分解层数

    # ===== 2. 直接计算幅度 =====
    amplitude = raw_signal  # 取绝对值
    ...
    adaptive_factor = np.clip(1 + 0.5 * (kurt - 3), 0.8, 1.5)  # 正态分布峰度为3
    high_threshold = 3
...
def low_freq_compensation(Insignal,eog_estimate, fs=250):
    """全局低频补偿"""
    # 提取超低频成分(0.5-2Hz)
    sos = signal.butter(4, [0.5, 4], 'bandpass', fs=fs, output='sos')
    low_comp = signal.sosfiltfilt(sos, Insignal)

    # 动态幅度匹配
    comp_scale = np.percentile(np.abs(low_comp), 95) / np.percentile(np.abs(eog_estimate), 95)
